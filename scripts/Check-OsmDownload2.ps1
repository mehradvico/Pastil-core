...
$pwFile = Join-Path ([System.IO.Path]::GetTempPath()) ("pastil-osmchk2-{0}.tmp" -f ([guid]::NewGuid().ToString('N')))
...
try {
    ...
    $output = & $plinkExe -batch -ssh -l $ServerUser -pwfile $pwFile $ServerHost "cat /root/osm-build/iran-latest.osm.pbf; echo; echo '===HEADERS==='; curl -sI https://download.openstreetmap.fr/extracts/asia/iran-latest.osm.pbf" 2>&1
