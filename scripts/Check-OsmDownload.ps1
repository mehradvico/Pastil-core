...
$pwFile = Join-Path ([System.IO.Path]::GetTempPath()) ("pastil-osmchk-{0}.tmp" -f ([guid]::NewGuid().ToString('N')))
...
try {
    ...
    $output = & $plinkExe -batch -ssh -l $ServerUser -pwfile $pwFile $ServerHost "cat /root/osm-build/iran-latest.osm.pbf; echo; echo '===VERBOSE==='; curl -v -o /dev/null https://download.geofabrik.de/asia/iran-latest.osm.pbf 2>&1 | head -30" 2>&1
    $ErrorActionPreference = $prevPref
    $output | ForEach-Object { if ($_ -is [System.Management.Automation.ErrorRecord]) { Write-Host $_.Exception.Message } else { Write-Host $_ } }
} finally {
