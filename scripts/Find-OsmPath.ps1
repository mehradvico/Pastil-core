...
$pwFile = Join-Path ([System.IO.Path]::GetTempPath()) ("pastil-findosm-{0}.tmp" -f ([guid]::NewGuid().ToString('N')))
...
try {
    ...
    $output = & $plinkExe -batch -ssh -l $ServerUser -pwfile $pwFile $ServerHost "curl -s https://download.openstreetmap.fr/extracts/asia/ | grep -i iran" 2>&1
