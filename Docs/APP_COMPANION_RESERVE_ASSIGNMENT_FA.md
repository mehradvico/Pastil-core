...
## تخصیص به همه‌ی همکاران کلینیک + تخصص‌های مرتبط با خدمت (۱۴۰۵/۰۷/۰۳)

مشکل قبلی: همکاری که «تخصص» داشت ولی هنوز به آن خدمت (مثلاً زایمان) **متصل نشده بود** در فهرست تخصیص نمی‌آمد و صاحب کلینیک نمی‌دانست چرا. تخصص فقط برچسب عنوان شغلی است؛ اتصال واقعی همکار به خدمت (`CompanionAssistanceUser`) جداست.

### فهرست همکاران

```http
GET /api/Companion/CompanionReserveAssignee/{reserveId}?includeAll=true
```

- بدون `includeAll` رفتار قبلی حفظ است (فقط همکاران متصل به همان خدمت)؛ اپ‌های قدیمی بدون تغییر کار می‌کنند.
- با `includeAll=true` **همه‌ی همکاران فعال و تأییدشده‌ی کلینیک** برمی‌گردند (همین‌طور برای `/api/Admin/CompanionReserveAssignee/{reserveId}`).
- فیلدهای جدید هر آیتم:

| فیلد | معنی |
|---|---|
| `companionUserId` | شناسه‌ی عضویت در کلینیک (همیشه هست) |
| `companionAssistanceUserId` | شناسه‌ی اتصال به همین خدمت؛ `null` وقتی هنوز متصل نیست |
| `isLinkedToService` | آیا به همین خدمت متصل است |
| `expertises` | همه‌ی تخصص‌های همکار (تخصص اصلی اول)؛ `expertiseName` فقط تخصص اصلی است |
| `isRecommended` | یکی از تخصص‌های همکار با تخصص‌های مرتبطِ همین خدمت یکی است |

ترتیب: مسئول فعلی، پیشنهادی‌ها، متصل‌ها، بقیه (بر اساس نام). فهرست تداخل زمانی را فیلتر **نمی‌کند**؛ تداخل هنگام ثبت بررسی می‌شود.

### ثبت تخصیص

```http
PUT /api/Companion/CompanionReserveAssign
{ "id": 120, "companionAssistanceUserId": 18 }     // همکار متصل
{ "id": 120, "companionUserId": 7 }                // همکار غیرمتصل
```

یکی از این دو لازم است. با `companionUserId` اگر همکار به این خدمت وصل نباشد، بک‌اند در همان تراکنش اتصال را می‌سازد (یا اگر غیرفعال بود فعال می‌کند). برای نمایندگی «شخصی» (`IsPersonal`) این مسیر `AccessDenied` می‌دهد.

### تخصص‌های مرتبط با خدمت (پنل ادمین)

- جدول جدید `AssistanceExpertises` (یکتا روی `AssistanceId + ExpertiseId`).
- `GET /api/Admin/AssistanceExpertise/{assistanceId}` و `PUT /api/Admin/AssistanceExpertise` با بدنه‌ی `{ assistanceId, expertiseIds: [] }` (جایگزینی کامل، حداکثر ۲۰).
- فقط برای «پیشنهاد» است؛ قاعده‌ی الزام‌آور نیست و صاحب کلینیک همچنان می‌تواند هر همکاری را انتخاب کند.
- پنل: صفحه‌ی ویرایش خدمت ← «تخصص‌های مرتبط».

### دیتابیس

Migration: `AddAssistanceExpertises` (فقط ساخت جدول). ترتیب دیپلوی: مایگریشن ← بک‌اند ← وب‌اپ/پنل.
