...
| ۶ | پنل ادمین (فقط خواندن): `GET /api/Admin/ConsultationPurchase` (جستجو + `{id}`) و `GET /api/Admin/ConsultationPackage` (کلینیک‌های دارای بسته‌ی فعال) و `/{companionId}` (ماتریس)؛ ثبت در `AdminPermissionCatalog` (گروه «مدیریت نمایندگان»)؛ صفحه‌های پنل `/admin/consultationpurchase` و `/admin/consultationpackage` + مستندات + چک‌لیست دیپلوی (بند ۱۲) | ✔ انجام شد |
