    ...
    public class PastilAiChatService : IPastilAiChatService
    {
        ...
        public async Task<BaseResultDto<PastilAiAskResultDto>> AskAsync(long userId, PastilAiAskDto dto, CancellationToken cancellationToken)
        {
            ...
            try
            {
                ...
                foreach (var attempt in routed.Attempts)
                {
                    await _context.PastilAiProviderAttempts.AddAsync(new PastilAiProviderAttempt
                    {
                        MessageId = assistant.Id,
                        Provider = attempt.Provider,
                        Model = attempt.Model,
                        AttemptOrder = attempt.Order,
                        Status = attempt.Response.IsSuccess ? PastilAiProviderAttemptStatus.Succeeded : PastilAiProviderAttemptStatus.Failed,
                        StartDateUtc = attempt.StartDateUtc,
                        EndDateUtc = attempt.EndDateUtc,
                        DurationMilliseconds = (long)(attempt.EndDateUtc - attempt.StartDateUtc).TotalMilliseconds,
                        HttpStatusCode = attempt.Response.HttpStatusCode,
                        ErrorCode = attempt.Response.ErrorCode,
                        ErrorMessage = Truncate(attempt.Response.ErrorMessage, 2000),
            ...
            catch (Exception ex)
            {
                ...
                await ReleaseQuotaAsync(userId, inputType, CancellationToken.None);
                return new BaseResultDto<PastilAiAskResultDto>(false, ex.Message, null);
            }
        }
        ...
        private async Task<PastilAiQuotaDto> GetQuotaInternalAsync(long userId, CancellationToken cancellationToken)
        {
            ...
            {
                PlanCode = plan.Code, PlanName = plan.Name, SubscriptionEndDateUtc = subscription?.EndDateUtc,
                UsedChats = usage?.ChatCount ?? 0, UsedImages = usage?.ImageCount ?? 0,
                UsedAudio = usage?.AudioCount ?? 0, UsedVideo = usage?.VideoCount ?? 0,
                DailyChatLimit = plan.DailyChatLimit, DailyImageLimit = plan.DailyImageLimit,
                DailyAudioLimit = plan.DailyAudioLimit, DailyVideoLimit = plan.DailyVideoLimit
            };
        }
        ...
        {
            Id = x.Id, UserId = x.UserId, UserFullName = $"{x.User.FirstName} {x.User.LastName}".Trim(),
            UserMobile = x.User.Mobile, Title = x.Title, CreateDateUtc = x.CreateDateUtc, UpdateDateUtc = x.UpdateDateUtc,
        ...
        {
            Id = x.Id, Role = x.Role, Status = x.Status, InputType = x.InputType, Scope = x.Scope,
            Content = x.Content, Provider = x.Provider, Model = x.Model, CreateDateUtc = x.CreateDateUtc,
            DurationMilliseconds = x.DurationMilliseconds,
            Attachments = attachments?.Select(a => new PastilAiAttachmentDto
            {
                PictureId = a.PictureId, FileId = a.FileId, Type = a.Type,
                Url = a.Picture != null
            ...
            ProviderAttempts = includeAttempts && x.ProviderAttempts != null
                ...
                {
                    Provider = a.Provider, Model = a.Model, AttemptOrder = a.AttemptOrder, Status = a.Status,
                    DurationMilliseconds = a.DurationMilliseconds, HttpStatusCode = a.HttpStatusCode,
